...
`take-okm`氏の公開マニュアル（`http://take-okm.a.la9.jp/oudia/...`）は調査時点でほぼ404（個別ページのみ一部生存）。上記階層構造は実ファイル（`高根鉄道28分パターンby Vague.oud2`、Noout外）を一時解析スクリプトで機械的に解析して確定させたもの。

### `EkiJikoku`（時刻本体）: 大筋解読済み・出典あり（`lib/oudParser.js`の`decodeEkiJikoku`参照）

**一次情報源が見つかった**: OuDiaSecond公式ブログの「[OuDiaSecondV2 マニュアル目次](https://oudiasecond.seesaa.net/article/467614009.html)」下部「＜OuDiaSecondのファイル形式＞」に、バージョンごとの「ファイル形式変更点」記事へのリンクが列挙されている。そのうちVer2.00.05の記事（`FileType=OuDiaSecond.1.05`→`1.07`）に、まさにEkiJikokuの書式が明記されていた:

```
EkiJikoku=[0-2];[hhmmss]/[hhmmss]$[0-],…
```

- **カンマがセグメント区切り**（駅の並び順どおり、1駅=1セグメント）。空セグメント（連続するカンマ）はその駅にデータなし。
- 1セグメントの書式は `駅扱いコード(0-2);時刻[/時刻2]$番線`。`駅扱いコード`と`番線`の正確な意味は変更点記事に明記されておらず未使用（TLINEのデータモデルは着発時刻だけで十分なため）。`/`がなければ単一時刻のみ（着のみ or 発のみ、駅の`Ekijikokukeisiki`表示形式で判別）。
- 時刻トークンは4桁=HHMM（秒00省略）、6桁=HHMMSS（例: `1619`=16:19:00、`204350`=20:43:50）。
- **FileType 1.05〜1.17までの全バージョン変更点記事を確認したが、この書式はVer2.00.05での導入以降変更されていない**（`Diagram/`内のサンプルは1.09〜1.15の範囲、いずれもこの書式で正しくデコードできる）。

以前の解読メモでは「`$`が駅区切り」という誤った仮説を元にしていた（`高根鉄道28分パターンby Vague.oud2`の巻き添え列車ではconfident:trueが0件だった）。上記の正しい文法（カンマが駅区切り）に修正した結果、同ファイルで28列車中15列車がconfident:trueにデコードできるようになった（`碧洛電車.oud2`も3→5/6に改善）。

**現状の精度**: 全区間走破する列車（カンマ区切り数=駅数と一致）は確信度高く(`timesConfident: true`)デコードできる。区間の一部だけ走る列車（短距離パターンダイヤ等）は、どの駅から始まるかがデータに明示されていないため、複数の整合するオフセット候補がある場合`timesConfident: false`になる。UIへの接続（インポートボタン等）は**まだ未実装**——`timesConfident: false`のデータをそのまま見せると誤解を招くため。残課題（オフセット精度向上・複数Dia対応・UI接続・エクスポート・駅扱いコード/番線の意味の特定等）は[issue #1](https://github.com/Illett0/TLINE/issues/1)に集約。
