...
Rosen.
  ...
  Dia.  (今回見たファイルは1つだけだったが複数ダイヤの可能性あり)
    DiaName=...
    Kudari.
      Ressya.  (繰り返し)
        Houkou=Kudari
...
`$`区切りが駅ごとのセグメントだろうという仮説で調べたが、単純に「駅数と$区切り数が一致する」ケースは確認できなかった（同じ路線・15駅に対し、列車ごとに$区切り数が4〜8とバラバラ）。観察できたパターン:

- 停車する駅のセグメントは`トラック番号;時刻[/時刻2]`の形（例: `1;204630`、`1;2048/204840`＝着/発）。時刻は6桁`HHMMSS`（`204630`=20:46:30）。
- 通過する駅（?）のセグメントは時刻なし、カンマ区切りの数値のみ（例: `0,1`）。
- セグメント内にさらにカンマで区切られた複数値が入ることがある（例: `,,2;204350`）。番線（`EkiTrack2`）が複数ある駅で、番線ごとのスロットを表している可能性が高いが未確定。
- 末尾に駅と無関係そうな単独の数値（例: `$1`）が付くことがある。

**結論**: 実データ（既存の込み入ったダイヤ）から逆算するには情報が足りない。次回は、**OuDiaSecondの実アプリで「駅3つ・列車1本・全駅停車・秒まで指定した時刻」のような最小構成のテストファイルを作ってもらい**、それを解読の基準にするのが確実（ユーザー本人に依頼予定）。中途半端な解読のままインポート機能に組み込むと誤った時刻を表示しかねないため、**この部分は未実装のまま保留**し、階層構造・駅一覧・列車番号/方向の抽出はできる状態で止めてある。
