...
- ヘッダーのボタンでライト/ダークテーマを切り替え可能（設定は端末に保存）。入出庫記号（○出区/▽入区）・運用番号ラベルの表示は、`.oud2`の`Operation`プロパティ（公式仕様非公開・統計的推定）から復元し、オン/オフを切り替えられる。列車運用のつなぎ線（同一運用の列車同士を接続する表現）は信頼できる根拠が得られず未実装。
