...
//   to actually re-draw a connecting line between the two trains: searching
//   every stop (optionally cross-checked by matching track/番線 too, per
//   the project owner's recollection) in every Dia of every sample for a
//   matching timestamp found no candidate at all, so drawing a connection
//   would risk linking the wrong trains outright. Project owner is
//   independently re-checking against the real connection in OuDiaSecond's
//   own UI; until then only the depot-boundary signal and the raw
//   operation-number string (where present) are used.
