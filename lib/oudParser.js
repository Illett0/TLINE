...
function inferOperationChains(trains) {
  ...
  trains.forEach((train, index) => {
    ...
    // リンクまで失われる）ことが判明したため。depotWorkの有無だけを条件に
    // するのではなく、issue #14に記録した「到着時刻→相手のdepotWork時刻」
    // という、より精密な間隔を使う方向性の方が有望。
    if (depTimeToken && train.chainPrevIndex == null) {
      departures.push({ index, stationIndex: first.stationIndex, track: first.track, time: timeToSeconds(depTimeToken) });
    }
    if (arrTimeToken && train.chainNextIndex == null) {
      arrivals.push({ index, stationIndex: last.stationIndex, track: last.track, time: timeToSeconds(arrTimeToken) });
    }
  });
