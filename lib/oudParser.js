...
function parseOperationEndpoint(raw, side) {
  ...
  if (!entry) return null; // rare/multi-train-only codes — not modeled, see above
  ...
  if (code === '5') return { linked: false, time: null, operationNumber };
...
// Second-pass (cross-track) window — see PASSES below.
const CROSS_TRACK_MIN_GAP_SECONDS = 3 * 60;
const CROSS_TRACK_MAX_GAP_SECONDS = 30 * 60;
...
// Ranks a compatible track pair: 2 = both sides recorded the SAME real
// (nonzero) track — an actual confirmed-same-platform continuation. 1 =
// compatible only because one side is 0 (番線未指定) — plausible but not
// confirmed. 0 = incompatible (only reachable in pass 2, which doesn't
// filter on this at all). Used to break ties by real-platform-evidence
// FIRST, raw time-gap second — see the 2026-07-15 fix below for why pure
// smallest-gap wasn't enough.
function trackMatchTier(t1, t2) {
  if (t1 === t2 && t1 !== 0) return 2;
...
function parseDiagram(text) {
  ...
  const dias = allChildren(rosen, 'Dia').map((dia, diaIndex) => {
    ...
    enforceRecordedNumberBoundaries(trains);
