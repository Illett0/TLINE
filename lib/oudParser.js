...
// 駅扱いコード（chunkの";"より前の数値）→ TLINEの`stopType`。NOTES.md
// 「駅扱いコード(0-2): 意味が判明」参照（プロジェクトオーナーのOuDiaSecond
// UI操作経験より確認済み）: 0=運行なし（実データに一度も出現しない）、
// 1=停車、2=通過。0や未知の値は素通し扱い（stopTypeなし＝従来通りの表示）。
function stopTypeFromCode(code) {
...
function parseStationChunk(chunk) {
  if (!chunk) return { arrival: null, departure: null, track: null, stopType: null };
  const semi = chunk.indexOf(';');
  if (semi === -1) return { arrival: null, departure: null, track: null, stopType: null }; // handling code with no time attached

  const stopType = stopTypeFromCode(chunk.slice(0, semi));
  ...
  if (dTok !== undefined) {
    ...
    return { arrival: parseTimeToken(aTok), departure: parseTimeToken(dTok), track, stopType };
  }
  ...
  return { arrival: time, departure: time, track, stopType };
}
...
// { stops: [{stationIndex, arrival, departure, track, stopType}], confident } —
// `stopType` is 'stop' | 'pass' | null (駅扱いコード, see stopTypeFromCode) —
...
// found via a Noout sample file — real data, see NOTES.md — a 21:00-24:00+
// Dia whose past-midnight trains are stored as plain 00:xx; without the shift
...
// 2026-08-08: 明示的な駅扱いコード（"2;..."のような通過エントリ）が記録
// されていなくても、その列車の最初〜最後の停車の間にあるのに全くデータが
// 無い駅は、実質的に通過していると推測できる（プロジェクトオーナー確認・
// 実データ確認: Nooutのある種別の列車は途中のある駅に一切stopsエントリを
// 持たないが、実際には通過している——詳細はNOTES.md参照）。ただし支線・分岐
// （issue #6、station.branchFromIndex）が絡む区間は「その駅を通らない
// （別区間を走る）」可能性と区別が付かないため、対象区間に分岐関連の駅が
// 1つでも含まれる場合はこの推測を行わない——見逃す方に倒す（誤って
// "通過"と表示するより、従来通り空欄のままの方が安全）。
function fillImplicitPasses(stops, stations) {
  if (stops.length < 2) return stops;
  const firstIndex = stops[0].stationIndex;
  const lastIndex = stops[stops.length - 1].stationIndex;
  const hasBranchInRange = stations.slice(firstIndex, lastIndex + 1).some((s) => s.branchFromIndex != null);
  if (hasBranchInRange) return stops;

  const byIndex = new Map(stops.map((s) => [s.stationIndex, s]));
  const filled = [];
  for (let i = firstIndex; i <= lastIndex; i++) {
    filled.push(byIndex.get(i) ?? { stationIndex: i, arrival: null, departure: null, track: null, stopType: 'pass' });
  }
  return filled;
}
...
function decodeEkiJikoku(raw, stations, kitenSeconds = 0) {
  ...
  chunks.forEach((chunk, i) => {
    ...
    // 時刻が両方とも空でも、駅扱いコードだけは記録されている稀なケース
    // （例: "2;$3"のように通過コードはあるが時刻トークン自体が空）は、
    // 「この駅は通らない」（chunkが完全に空）と区別するために停車として
    // 残す——stopCellHtml側で「レ」表示に使う（2026-08-08要望）。
    if (!parsed.arrival && !parsed.departure && !parsed.stopType) return;
    ...
    stops.push({ stationIndex: i, arrival: parsed.arrival, departure: parsed.departure, track: parsed.track, stopType: parsed.stopType });
  });
  return { stops: fillImplicitPasses(stops, stations), confident };
}
...
//   major terminus/depot station (Noout files: several such stations, see
//   NOTES.md; 高根鉄道TM: 高根) regardless of OuterTerminal usage, which rules out a
...
//   apparently single-track ones (each Noout station), consistent with it being a
...
// per side (2026-07-15, surveyed against three Noout files — see NOTES.md —
// from a different author than the 高根鉄道 set, so the two
...
//     records; the Noout files write it directly as `$90`/`$06` on plain
...
//   A side: `$<数値>` is a small CONSTANT per file (TM: always `0`; Noout:
...
function parseOperationEndpoint(raw, side) {
  ...
  // 出区/入区情報として読める（Noout系ファイルでは "5/$74,0/0$2050/..." のように
...
// 30min→60min (2026-07-15, project owner report: a Noout sample's real
// turnback with a 35m50s layover, track0=0 (see NOTES.md), and both ends are
...
// owner報告のNooutのある実データ（詳細はNOTES.md参照）で発覚: 両者とも
// 番線0で935秒差の組が、弱いtier1のまま扱っていたため、番線3どうしで
// 520秒差の別の組（こちらもtier1）に競り負けていた。0===0をtier2に
// 格上げすると、935秒差でもtier優先で本来のペアが勝ち、相互最近傍も
// 成立する（もう一方は自分の番線3と一致する他の候補を探すことになる）。
...
// by the Noout files — a different author whose data records REAL tracks on
...
//     track and depart from a different one near-instantly, and a Noout
//     file's data is full of such coincidences (two unrelated trains
//     arriving/departing the same station on different tracks within the
//     same minute; naive any-track nearest would wrongly join them). The
//     30-minute ceiling covers observed real terminal layovers (a Noout
//     station's same-track 24-25min turnbacks that pass 1's 15min cap
//     missed — see NOTES.md). Restricting pass 2 to
...
// `startIndex`が属するチェーンで、そこまでに判明している運用番号を返す
// （無ければnull）。propagateOperationNumbers（先頭から前方へ辿って
// 「最後に見つかった記録済み番号」を引き継ぐ）と同じ値を、startIndexから
// chainPrevIndexを遡って「最初に見つかった記録済み番号」を返すことで
// 効率的に求める（後ろ向きに辿って最初に見つかる＝前方伝播で最後に更新
// された値と等価）。2026-08-08「設計変更」対応: enforceRecordedNumber
// Boundariesのような後付けの切断ではなく、マッチングの候補選定自体で
// この関数を使い、既に分かっている番号と矛盾する候補を最初から除外する
// （詳細はinferOperationChains内のnearestDeparture/nearestArrival参照）。
function resolvedChainNumberSoFar(trains, startIndex) {
  ...
  while (index != null && !visited.has(index)) {
    ...
    index = t.chainPrevIndex != null ? t.chainPrevIndex : null;
  }
  return null;
}

// `trains`が既に持つchainNextIndex/chainPrevIndexは尊重し、まだ無い
// エンドポイントだけを対象にマッチングする（2026-08-08、下記
// 「マッチング→矛盾チェックの反復」対応で複数回呼べるようにするための
// 前提——既にリンク済みのエンドポイントを毎回作り直すと、enforceRecorded
// NumberBoundariesが切っていない正しいリンクまで別の候補に奪われかねない）。
function inferOperationChains(trains) {
  ...
  trains.forEach((train, index) => {
    ...
    // depotWorkを持つ端点を候補プールから一律/非対称に除外する案は
    // どちらも試したが撤回した（issue #14参照）。一律除外は全ファイル
    // 横断でtotalChainLinks 1462→1305（-157）と大悪化、「出発側にdepotWork
    // があり到着側に無ければ除外」という非対称版もtotalChainLinks
    // 1462→1443（-19）で、しかもその19件がNooutのある駅・番線での正常な
    // 折り返し記録慣習（出発側にしかdepotWorkが付かないのがこの駅の通常
    // パターン）を軒並み壊してしまう（既に修正済みのリンクまで失われる
    // ケースを含む——詳細はNOTES.md）ことが判明したため。
    //
    // 「depotWorkのtrack/arrivalそのものを発着番線・時刻の代わりに使う」案
    // も試したが撤回した（2026-08-08、issue #14）: Nooutの一部の駅ではdepotWork
    // のtrackがほぼ常に固定値（例: 0）で、これをtrackMatchTierの0===0昇格
    // ルールに通すと無関係な同系統列車同士まで「確定的一致」に見えてしまい
    // 誤結合の原因になった上、track=0を除外してもtime側の置き換えだけで
    // ある間隔運行パターン（約20組、詳細はNOTES.md）が丸ごと別の
    // （正しいとも間違っているとも検証できない）組み合わせへ再配線される
    // 副作用が見つかった——depotWorkの滞留時間が列車ごとに微妙に異なるため、
    // 公開ダイヤ上は等間隔だった候補集団の相対順序がtime置き換えだけで
    // 崩れてしまう。gapの基準時刻自体は公開ダイヤのまま変えず、
    // 「depotWorkのtrack（0以外の実数値）が相手側の実際の番線と一致する
    // 場合だけtier2に格上げする」という、trackだけを見るより限定的な形に
    // 絞った（下記のdepotTrackTier）。
    const originDepotTrack = train.operation && train.operation.origin && train.operation.origin.depotWork && train.operation.origin.depotWork.track;
    const terminalDepotTrack = train.operation && train.operation.terminal && train.operation.terminal.depotWork && train.operation.terminal.depotWork.track;
    if (depTimeToken && train.chainPrevIndex == null) {
      departures.push({
        index,
        stationIndex: first.stationIndex,
        track: first.track,
        time: timeToSeconds(depTimeToken),
        depotTrack: originDepotTrack || null, // 0/nullはどちらも「実数値なし」として扱う
      });
    }
    if (arrTimeToken && train.chainNextIndex == null) {
      arrivals.push({
        index,
        stationIndex: last.stationIndex,
        track: last.track,
        time: timeToSeconds(arrTimeToken),
        depotTrack: terminalDepotTrack || null,
      });
    }
  });
  ...
  // feedback on a Noout sample — see NOTES.md for the exact figures: an
  // arrival on a real track, and the intended continuation departing shortly
  // after on that SAME track (515s gap) — but pure-gap picking instead chose
  // a different train's arrival on a wildcard track, 65s closer purely by
  // chance. A wildcard match isn't evidence of anything
  ...
  // depotWorkの実番線（0/nullは既にdepotTrackから除外済み — 上のtrains.forEach
  // 参照）が相手側の公開ダイヤ上の番線と一致するなら、raw同士のtrackMatchTier
  // と同じtier2として扱う。gap自体はここでは変えない（公開ダイヤの値のまま）
  // ——理由は上の長いコメント参照。実例（Noout、詳細はNOTES.md）: 到着列車の
  // 到着番線と、出発列車のdepotWork.trackが一致 → tier2に格上げされ、gapで
  // 競っていた別候補（どちらの番線とも一致しない）を確定的に上回る。
  function depotTrackTier(depotTrack, counterpartTrack) {
    return depotTrack != null && depotTrack === counterpartTrack ? 2 : 0;
  }
  // 既に判明している運用番号との矛盾を、マッチングの候補選定自体で除外する
  // （2026-08-08設計変更、issue #14）。従来は近接ヒューリスティックだけで
  // 相互最近傍を確定させ、矛盾があれば`enforceRecordedNumberBoundaries`が
  // 後から切っていたが、切られた側が再挑戦する頃には本来の相手が
  // 別の候補に先に取られてしまっているケースがあった（実例はNOTES.md参照:
  // ある到着列車の最短候補は別チェーンの運用番号を持つ列車で、その到着
  // 列車の属するチェーンの番号と矛盾するため後から切られていたが、その
  // 時点で本来の相手は既に別の候補と確定済みだった）。候補の時点で弾けば、
  // 最初から矛盾する候補を試さず本来の相手を選べる。`arrivalNumber`は
  // arrival側で1回だけ解決すればよい
  // （候補ごとに変わらないため、ループの外で計算）。
  function nearestDeparture(arrival, pool, pass) {
    if (arrival.track == null) return null;
    const arrivalNumber = resolvedChainNumberSoFar(trains, arrival.index);
    let best = null;
    for (const d of pool) {
      if (d.index === arrival.index || d.stationIndex !== arrival.stationIndex || d.track == null) continue;
      const dRecorded = trains[d.index].operation && trains[d.index].operation.origin && trains[d.index].operation.origin.operationNumber;
      if (arrivalNumber != null && dRecorded && dRecorded !== arrivalNumber) continue;
      const tier = Math.max(
        trackMatchTier(d.track, arrival.track),
        depotTrackTier(d.depotTrack, arrival.track),
        depotTrackTier(arrival.depotTrack, d.track)
      );
  ...
  function nearestArrival(departure, pool, pass) {
    ...
    const departureRecorded =
      trains[departure.index].operation && trains[departure.index].operation.origin && trains[departure.index].operation.origin.operationNumber;
    let best = null;
    for (const a of pool) {
      if (a.index === departure.index || a.stationIndex !== departure.stationIndex || a.track == null) continue;
      const arrivalNumber = resolvedChainNumberSoFar(trains, a.index);
      if (arrivalNumber != null && departureRecorded && departureRecorded !== arrivalNumber) continue;
      const tier = Math.max(
        trackMatchTier(a.track, departure.track),
        depotTrackTier(a.depotTrack, departure.track),
        depotTrackTier(departure.depotTrack, a.track)
      );
      if (pass.requireCompatibleTracks && tier === 0) continue;
  ...
  for (const pass of PASSES) {
    // 相互最近傍のフォールバック（2026-08-08要望）: 元々は各エンドポイントの
    // 「一番近い候補」同士が一致した場合しかリンクにしていなかったが、
    // 一番近い候補が必ずしも自分を選び返してくれるとは限らない（実例は
    // NOTES.md参照: ある到着列車の最近傍は別の列車だが、その別の列車自身の
    // 最近傍は別にあるため不成立——その結果、本来繋がるべき2番目に近い
    // 候補も見捨てられていた）。「振られた特定の組み合わせ」だけをこのpass内で除外し、
    // 除外後の残り候補で相互最近傍を再計算する、というのを進展が無くなる
    // まで繰り返す（Gale-Shapley安定マッチングの簡易版——除外は特定の
    // ペア単位で行い、エンドポイント自体を候補プールから外すわけではない
    // ので、他の相手にとっては引き続き候補になれる）。
    const rejectedPairs = new Set(); // `${arrivalIndex}:${departureIndex}`
    let roundArrivals = freeArrivals;
    let roundDepartures = freeDepartures;
    let progressed = true;
    while (progressed) {
      progressed = false;
      const nextFor = new Map(); // arrival's train index -> best departure match（このラウンドの除外を反映）
      for (const a of roundArrivals) {
        const pool = roundDepartures.filter((d) => !rejectedPairs.has(`${a.index}:${d.index}`));
        const match = nearestDeparture(a, pool, pass);
        if (match) nextFor.set(a.index, match);
      }
      const prevFor = new Map(); // departure's train index -> best arrival match
      for (const d of roundDepartures) {
        const pool = roundArrivals.filter((a) => !rejectedPairs.has(`${a.index}:${d.index}`));
        const match = nearestArrival(d, pool, pass);
        if (match) prevFor.set(d.index, match);
      }
      const matchedArrivalIndex = new Set();
      const matchedDepartureIndex = new Set();
      for (const [fromIndex, next] of nextFor) {
        ...
        if (back && back.index === fromIndex) {
          ...
          matchedArrivalIndex.add(fromIndex);
          matchedDepartureIndex.add(next.index);
          progressed = true;
        } else {
          // この矢印（到着側からの提案）だけを「振られた」として除外し、
          // 次ラウンドでこの到着はこの出発以外の候補を試す。出発側の
          // 志向（prevFor）はここでは除外対象にしない——今ラウンドで別の
          // より近い到着に気を取られていただけの可能性があり、時期尚早に
          // 除外すると、後で本来成立すべき組み合わせまで潰してしまうため。
          rejectedPairs.add(`${fromIndex}:${next.index}`);
          progressed = true;
        }
      }
      roundArrivals = roundArrivals.filter((a) => !matchedArrivalIndex.has(a.index));
      roundDepartures = roundDepartures.filter((d) => !matchedDepartureIndex.has(d.index));
    }
...
// （2026-07-15、project owner報告: Nooutのある実データで、ある列車が3駅間
// 3ブロックに分かれている——詳細はNOTES.md参照）。分割点のOperationは
// 両側ともコード5（非リンク＝
...
// head. Measured across the Noout files before this pass existed, the
// proximity heuristic alone merged clearly-different operations at busy
// terminals — e.g. a train recorded as one 運用番号 chained into a train
// whose own 出区記録 says a DIFFERENT 運用番号, and 19 of 104 links in one
// file flowed into a numbered 出区 point this way (see NOTES.md for the
// exact examples) — while the matching-number cases it also produced are
// consistent continuations worth keeping. Chains with no number information on either side are left
...
//
// 2026-08-08修正: 元の条件は「番号不明のまま(number===null)、明示的な
// 出区点に流れ込んだ」場合も「別運用の可能性」として切っていたが、
// 「これまで番号が分かっていなかった（=比較材料が無い）」のと「既に別の
// 番号だと分かっている（=本当の矛盾）」は区別すべきという指摘を受けた
// （実例: Nooutのある4本連続のチェーンで、末尾寄りの1本の出区記録にだけ
// 運用番号が付いており、それまでの区間に番号情報が一切無かった手前側の
// リンクまで切られていた——詳細はNOTES.md参照）。全サンプル
// ファイル横断で実際にこの条件が発火した83件を「切断理由」で分類したところ、
// 「既知の番号との矛盾」80件・「不明→番号発見」3件で、後者はいずれも
// 単純に比較材料が無かっただけ（真に矛盾する番号との衝突ではない）と
// 確認できたため、`number != null`（＝既に何らかの番号が分かっている場合
// のみ）を条件に追加——「不明」と「別の番号」を区別し、真の矛盾がある
// 場合のみ切るよう厳密化した。
// 戻り値: 実際に切ったリンクの本数（下記「マッチング→矛盾チェックの反復」
// が収束判定に使う）。
function enforceRecordedNumberBoundaries(trains) {
  ...
  trains.forEach((train, headIndex) => {
    ...
    while (index != null && !visited.has(index)) {
      ...
      const t = trains[index];
      const recorded = t.operation && t.operation.origin && t.operation.origin.operationNumber;
      if (recorded && number != null && t.chainPrevIndex != null && recorded !== number) {
        // 既に分かっている番号と矛盾する場合のみ切る——「まだ何も分かって
        // いなかった」場合まで疑わない（上記2026-08-08コメント参照）。
...
// 高根: 1番線/2番線/.../TST上/TST下; a Noout station's list uses non-numeric
// labels — station-specific, and NOT declaration-order==display-number in
// general, e.g. one Noout station's list starts at index 5 rather than 1,
// see NOTES.md). EkiJikoku's own `$N` track suffix (see parseStationChunk)
// is a 0-based index directly into THIS list — confirmed 2026-07-15 against
// project-owner-supplied ground truth from a Noout file (a specific train's
// recorded value resolved to the label the owner independently knew that
// train's real platform to be — see NOTES.md for the exact figures). Until
// this, renderer/app.mjs's timetable displayed the raw `$N` number itself,
// which is wrong whenever a station's tracks aren't literally numbered 1..N
// in declaration order — true of nearly every Noout station (circled
// numerals, 上本/下本, X/Y/E-style labels) and any renumbered/reordered case
// elsewhere.
...
// declared per-station (nested inside `Eki.`, not a Rosen-level list; a
// Noout file has two such stations, one a branch point — see NOTES.md for
// the exact counts/indices). These are the names OuDiaSecond's
...
//   - Code 0 is common (292 occurrences in one Noout file alone) and spread
//     across FIVE different stations, only one of which has an OuterTerminal
//     list at all — a real link would concentrate at just the file's two
//     OuterTerminal-bearing stations, not spread this widely.
...
function parseDiagram(text) {
  ...
  // OuDia/OuDiaSecond has no real branch/multi-line concept in its own file
  // format — a branch is expressed purely by convention: the junction
  // station's Eki. block is listed a *second* time later in the same flat
  // list, and branch trains' EkiJikoku just reference that later index (see
  // NOTES.md「支線・分岐の表現」, issue #6). Before this, that convention was
  // only ever inferred ad-hoc and per-train inside
  // renderer/diagramView.mjs's trainPolylineSegments (comparing consecutive
  // stops' station names) — no other consumer (timetable, future export,
  // issue #4's conflict check) had any way to know a station was a branch
  // continuation. Recording it here, once, as `branchFromIndex` (pointing
  // back at the *first* occurrence of that name) makes it an explicit,
  // shared fact about the station list instead of a renderer-only side
  // effect. This does NOT change distanceKm/layout — verified against
  // Diagram/image/06123.png (OuDiaSecond's own rendering of
  // 高根鉄道TM.oud2) that stacking the branch's stations below the main
  // line's cumulative distance, rather than resetting back to the junction,
  // is the *correct*, reference-matching behavior, not a bug.
  ...
  for (const eki of allChildren(rosen, 'Eki')) {
    ...
    const name = eki.props['Ekimei'] || '';
    const branchFromIndex = firstIndexByName.has(name) ? firstIndexByName.get(name) : null;
    ...
    stations.push({
      ...
      branchFromIndex, // index of this name's first occurrence, or null — see comment above
    });
  ...
  const dias = allChildren(rosen, 'Dia').map((dia, diaIndex) => {
    ...
    //
    // マッチング→矛盾チェックの反復（2026-08-08要望）: enforceRecorded
    // NumberBoundariesが切ったリンクの列車は、切られた直後は再びフリーに
    // なるが、inferOperationChainsをもう一度呼ばない限り新しい相手を
    // 探す機会が無い（実例はNOTES.md参照: ある列車の最近傍が運用番号の
    // 矛盾で切られた後、2番目に近い候補を試すには再マッチングが要る）。無限ループの
    // リスクを避けるため、切断が0件になるか5回に達するまでの反復に
    // 留める（「変化がなくなるまで」ではなく明示的な上限を設ける方針、
    // プロジェクトオーナー指示）。
    const MAX_CHAIN_ITERATIONS = 5;
    for (let i = 0; i < MAX_CHAIN_ITERATIONS; i++) {
      inferOperationChains(mergedTrains);
      const cutCount = enforceRecordedNumberBoundaries(mergedTrains);
      if (cutCount === 0) break;
    }
...
function toTlineDiagram(parsed, diaIndex) {
  ...
  const stations = parsed.stations.map((s) => ({
    ...
    // Set when this station is a branch continuation — see the comment
    // above parseDiagram's station-building loop (issue #6). Consumed by
    // renderer/diagramView.mjs (branch-seam divider + trainPolylineSegments)
    // and renderer/app.mjs (timetable row annotation).
    branchFromStationId: s.branchFromIndex != null ? stationId(s.branchFromIndex) : null,
  }));
  ...
  dia.trains.forEach((train, i) => {
    ...
    trains.push({
      ...
      stops: train.stops.map((s) => ({
        ...
        // 'stop' | 'pass' | null（駅扱いコード、decodeEkiJikoku参照）。
        // renderer/app.mjsのstopCellHtmlが通過駅を薄いグレー表示/「レ」
        // マークに使う（2026-08-08要望）。
        stopType: s.stopType,
      })),
    });
