...
// trains. `timesConfident` (per train), `track` (per stop), and `trainType`
// (per train — `{name, abbreviation, color, dashArray}` or null if the file
// has no Ressyasyubetsu match, e.g. hand-authored plan data) are carried
// through as extra (data-model-additive) fields; renderer/diagramView.mjs
// reads `trainType` to color/style each line, falling back to a plain
// default when it's null. `track` has no consumer yet (kept for a future
// track-aware conflict check, issue #4).
