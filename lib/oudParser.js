...
// 実データ確認: 館浜運転会司令v0414.oud2で末尾Aの特急（1603A等）は河原崎に
// 一切stopsエントリを持たないが、実際には通過している）。ただし支線・分岐
...
function inferOperationChains(trains) {
  ...
  // ——理由は上の長いコメント参照。1751→1750: 1751の到着番線=2、1750の
  // depotWork.track=2で一致 → tier2に格上げされ、gapで競っていた回1702A
  // （どちらの番線とも一致しない）を確定的に上回る。
  ...
  // 後から切っていたが、切られた側が再挑戦する頃には本来の相手（例:
  // 1750）が別の候補（回1702A）に先に取られてしまっているケースがあった
  // （1751の最短候補1787Bは運用番号86で、1751の属するチェーンの番号50と
  // 矛盾するため後から切られていたが、その時点で1750は既に回1702Aと
  // 確定済みだった）。候補の時点で弾けば、1751は最初から1787Bを試さず
  // 1750を選べる。`arrivalNumber`はarrival側で1回だけ解決すればよい
  ...
  for (const pass of PASSES) {
    ...
    // 一番近い候補が必ずしも自分を選び返してくれるとは限らない（例:
    // 1751の最近傍は1787Bだが、1787B自身の最近傍は別にあるため不成立——
    // その結果、1751と本来繋がるべき1750〈2番目に近い候補〉も見捨てられて
    // いた）。「振られた特定の組み合わせ」だけをこのpass内で除外し、
...
// （実例: 館浜運転会司令v0414.oud2の1750→1851→1850→1951相当のチェーンで、
// 1850の出区記録に運用番号「50」が付いているだけで、それまでの区間に
// 番号情報が一切無かった1851→1850のリンクまで切られていた）。全サンプル
...
function parseDiagram(text) {
  ...
  const dias = allChildren(rosen, 'Dia').map((dia, diaIndex) => {
    ...
    // 探す機会が無い（実例: 1751の最近傍1787Bが運用番号の矛盾で切られた
    // 後、2番目に近い1750を試すには再マッチングが要る）。無限ループの
