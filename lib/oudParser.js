...
// are required — track equality is what makes this trustworthy (same
// station, different track, within the window is just as likely two
...
function inferOperationChains(trains) {
  ...
  // Nearest same-station+track departure (nonnegative gap, within the
  // window) for each arrival, and the mirror search for each departure —
  ...
  function nearestDeparture(arrival) {
    ...
    for (const d of departures) {
      if (d.index === arrival.index || d.stationIndex !== arrival.stationIndex || d.track !== arrival.track) continue;
  ...
  function nearestArrival(departure) {
    ...
    for (const a of arrivals) {
      if (a.index === departure.index || a.stationIndex !== departure.stationIndex || a.track !== departure.track) continue;
