...
// Simple MRU list of recently opened/saved .tline.json project file paths,
...
// Moves `filePath` to the front (or adds it), deduped by path.
function touchRecentFile(userDataPath, filePath) {
  const list = readList(userDataPath);
  const rest = list.filter((e) => e.path !== filePath);
  const entry = { path: filePath, name: path.basename(filePath), lastOpenedAt: Date.now() };
